...
//---------------------------------------------------------------------
// Bitmap definitions and functions to manage space in the swapfile.
...
int swap_pagein(unsigned frame, int swap_offset) {
	...
	// Seek to position in the swap file where this page was stored
...
// in the swap file. Allocates space in the swap file for virtual page if needed.
// Input:  frame - the physical frame number (not byte offset in physmem)
//         swap_offset - the byte position in the swap file.
...
int swap_pageout(unsigned frame, int swap_offset) {
	...
	if (swap_offset == INVALID_SWAP) {
		if (bitmap_alloc(swapmap, &idx) != 0) {
			fprintf(stderr,"swap_pageout: Could not allocate space in the swapfile. Try running again with a larger swapsize.\n");
			return INVALID_SWAP;
		}
	...
	// Seek to position in the swap file where this page will be stored
	pos = lseek(swapfd, swap_offset, SEEK_SET);
	if (pos != swap_offset) {
		assert(pos == (off_t)-1);
