 ...
 *
 * The find_physpage() function is invoked to translate the virtual address
...
void replay_trace(FILE *infp) {
	...
	while(fgets(buf, MAXLINE, infp) != NULL) {
		if(buf[0] != '=') {
			...
			vaddr = vaddr & 0x0000000fffffffff; //have to mask higher bits to 0 to avoid out of bounds access
